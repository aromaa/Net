...
{{
    ...
    {{
        ...
        {(handler ? $"public partial void Handle(IPipelineHandlerContext context, in {packetType} packet);" : string.Empty)}
				...
				context.AddSource($"{symbol.ContainingNamespace}_{symbol.Name}_IIncomingPacketConsumer.g.cs", SourceText.From(stringBuilder.ToString(), Encoding.UTF8));
			}
		}
	}
	...
	{
		internal List<ClassDeclarationSyntax> ClassesWithAttributes { get; } = [];
