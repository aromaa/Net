...
{
    internal abstract class AbstractPipelineSocket : IPipelineSocket
    {
        ...
        private event SocketEvent<IPipelineSocket>? ConnectedEvent;
        private event SocketEvent<IPipelineSocket>? DisconnectedEvent;
        ...
        {
            await this.SendQueue!.SendAsync(ISendQueueTask.Create(data));
        }

        public Task SendPacketAsync<T>(in T packet)
        {
            return Task(ISendQueueTask.Create(packet));
        ...
        private void ProcessWriter(PipeWriter writer, ISendQueueTask task)
        {
            ...
            SocketPipelineContext pipeline = new SocketPipelineContext(this);
            PacketWriter packetWriter = new PacketWriter(writer);

            task.Write(ref pipeline, ref packetWriter);
        ...
        {
            public void Write(ref SocketPipelineContext context, ref PacketWriter writer);
        ...
        private sealed class SendQueueTask<T> : ISendQueueTask
        {
            ...
            private ref T RefValue => ref Unsafe.AsRef(this.Value);

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private ref TOther As<TOther>() => ref Unsafe.As<T, TOther>(ref this.RefValue);

            public void Write(ref SocketPipelineContext context, ref PacketWriter writer)
            {
                if (typeof(T) == typeof(ReadOnlyMemory<byte>))
                {
                    writer.WriteBytes(this.As<ReadOnlyMemory<byte>>().Span);
                }
                else
                {
                    context.ProgressWriteHandler(ref writer, this.Value);
                }
            }
        }
    }
